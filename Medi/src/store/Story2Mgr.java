...
public class Story2Mgr {
	...
	public boolean replyUpload(String id , int pos, String comment) {
		...
		try {
			con = pool.getConnection();
			sql= "select * from tblcartoon tblstoryreply_car where pos=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, pos);
			rs = pstmt.executeQuery();
			while(rs.next()) {
			...
			sql = "insert into tblcartoon tblstoryreply_car(id,pos,comment ,ref) values(?,?,?,?)";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, id);
			pstmt.setInt(2, pos);
...
public boolean replyDelete( int pos, String comment) {
		...
		try {
			con = pool.getConnection();	
			sql = "delete from tblcartoon tblstoryreply_car where pos =? and comment=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, pos);
