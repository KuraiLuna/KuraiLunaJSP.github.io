...
public class cartoonMgr {
	...
	public boolean cartoonUpload(HttpServletRequest req) {
		Connection con =null;
		...
		try {
			...
			sql = "insert tblcartoon(id, content, imgname,imgsize) values(?,?,?,?)";
	...
	//리스트
	public Vector<cartoonBean> cartoonList(){
		Connection con =null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = null;
		Vector<cartoonBean> vlist = new Vector<cartoonBean>();
		
		try {
			con = pool.getConnection();
			sql = "select * from tblcartoon";
			pstmt = con.prepareStatement(sql);
	...
	public Vector<cartoonBean> todaycartoonList(){
		Connection con =null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = null;
	...
	public void cartoonDelete( int snum) {
		...
		try {
			con = pool.getConnection();
				...
				sql = "select imgname from tblcartoon where snum =?";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, snum);
				rs = pstmt.executeQuery();
				...
				sql = "delete from tblcartoon where snum= ?";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, snum);
				pstmt.executeUpdate();	
	...
	public boolean replyUpload(String id , int pos, String comment) {
		...
		try {
			con = pool.getConnection();
			sql= "select * from tblstoryreply_car where pos=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, pos);
			rs = pstmt.executeQuery();
			while(rs.next()) {
			...
			sql = "insert into tblstoryreply_car(id,pos,comment ,ref) values(?,?,?,?)";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, id);
			pstmt.setInt(2, pos);
...
public boolean replyDelete( int pos, String comment) {
		...
		try {
			con = pool.getConnection();	
			sql = "delete from tblstoryreply_car where pos =? and comment=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, pos);
